...
- 初始化日期：2026-08-24
- 当前阶段：**P7 + P8 已交付（2026-09-07），版本 0.9.0b1，全阶段功能 61/61 done**。P7：Windows 用户级安装器 + 便携 zip + Release 通道（合同 docs/INSTALLER_RELEASE_CONTRACT.md + ADR 0006；frozen=production、token fragment 引导、SPA catch-all、单实例退出码 3/4、引擎 exe bridge 子命令打包入口、迁移前 SQLite backup 快照、Inno Setup 真机验收 16/16、release.yml 草稿制发布）。P8：docs/HARDENING_MATRIX.md 14 行失败模式到护栏、worker_runtime 诊断检查 + 首页排队横幅、脱敏诊断导出、故障注入回归、docs/SUPPORT.md。发布由 Owner 手动执行：bump 版本、push tag、CI 出草稿、审阅发布。
