...
- **设置双写(指针先行 + 串行化 + 补偿)**:保存 `report_directory` 时先原子写指针、
  ...
  不得交错成「指针=B、数据库=A」;数据库提交失败时**补偿恢复旧指针**(仅当指针确实
  被替换过)再返回 503——失败的保存绝不能留下「新指针 + 旧数据库」。指针写与补偿在
  事件循环内**同步执行**(不进线程池):两者构成不可分割的临界区——取消不会落在
  replace 与补偿之间,被取消的线程也不会迟到地用旧值覆盖补偿结果;设置保存低频,
  fsync 阻塞是可接受的代价。补偿覆盖**取消**(CancelledError 是 BaseException)且
  **按提交 verdict 补偿**(R10 四轮评审修订 2026-09-14):取消可落在 commit 的 await
  上而 SQLite 已持久化——`immediate_write_transaction` 经 `on_commit` 回调报告
  durable verdict,已提交的取消**不得**恢复旧指针(否则库=新、指针=旧分裂),只清理
  被替换的凭据槽;未提交的取消才恢复旧指针后传播。两个存储无法真正原子:指针只在
  删库后生效、settings 存活时以 settings 为准,补偿后的唯一残留窗口是「补偿写也
  失败」的双重磁盘故障,记日志暴露。
