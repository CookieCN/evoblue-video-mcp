...
**Rule**：可复现性只约束确定性产物；性能、内存、时序等测量值用阈值和有效域判断，不做跨运行逐字节相等。

## 25. 国内模型源不能只换域名——运行时格式、固定版本和逐文件身份必须一起解决

**Problem**：Qwen 官方为大陆用户推荐 ModelScope，但官方原始权重依赖 PyTorch/Transformers 或 vLLM；直接把官方 ModelScope 链接加进现有模型列表，安装后并不能被 EvoBlue 的轻量 sherpa-onnx Provider 加载。可用的国内 ONNX 导出又是多文件仓库，不是既有单归档下载合同。

**Root Cause**：把“下载可达”误当成“制品可运行”；模型来源、推理运行时、转换格式、版本冻结、断点续传和许可证是一个完整交付链，缺一项就只是 UI 假选项。

**Solution**：选用 sherpa-onnx 已原生支持的 Qwen3-ASR 0.6B INT8 格式，固定 ModelScope 导出提交；新增 `file-set` 合同，把远端路径→本地路径、每文件大小/SHA 与集合指纹全部冻结，Range 逐文件续传后经 staging 原子晋升。保留 Standard 正式默认，Qwen 等独立基准后再决定推荐地位。

**Rule**：新增模型安装选项前必须同时证明“国内可达、格式可加载、版本不可漂移、内容可校验、失败不半激活”；第三方转换制品未经项目基准不得借上游模型名自动继承质量结论。

## 26. 发行验证失败也必须回收测试进程——否则重跑会连接旧引擎

**Problem**：发行验证因模型数量旧断言失败后遗留打包引擎；下一轮在同一端口读到旧进程的健康响应，随后因数据目录不匹配出现 token 文件假失败。

**Root Cause**：`verify_release.py` 只在正常路径逐个 `stop()`，外层 `finally` 只删临时目录，断言抛出时跳过进程回收；固定端口令遗留进程能污染后续运行。
